...
export const getClassOverview = async () => {
  try {
    ...
    const response = await apiClient.get(`/class/overview?_t=${timestamp}`);
...
/**
 * Get class performance data for specific year and section
 * @param {number} year - Year (1, 2, 3, 4)
 * @param {string} section - Section (CSE A, CSE B, etc.)
 * @returns {Promise} Class performance data with subjects and students
 */
export const getClassPerformance = async (year, section) => {
  try {
    // URL encode the section to handle spaces and special characters
    const encodedSection = encodeURIComponent(section);
    
    const response = await apiClient.get(`/class/performance/${year}/${encodedSection}`);
    
    // The backend returns { success: true, data: {...} }
    // Return the full response so frontend can check success status
    return response.data;
  } catch (error) {
    console.error('Error fetching class performance:', error);
    throw new Error(
      error.response?.data?.error || 
      'Failed to fetch class performance data'
    );
  }
};
