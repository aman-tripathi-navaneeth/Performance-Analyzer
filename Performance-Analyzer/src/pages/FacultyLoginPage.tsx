...
const FacultyLoginPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Expected role can be 'faculty' or 'hod', just saving it entirely
            ...
            if (data.teacher.role === 'hod') roleDisplay = 'HOD';
            if (data.teacher.role === 'tpo') roleDisplay = 'TPO';
            toast.success(`${roleDisplay} login successful!`);

            // Based on role routing might be handled by the layout or we can force navigate based on existing structure.
            navigate('/faculty-dashboard');
        } catch (error: any) {
