...
def upload_subject_file():
    ...
    try:
        ...
        if not allowed_file(file.filename):
            ...
            return jsonify({
                'success': False,
                'error': 'Invalid file type. Only Excel files (.xlsx, .xls) and CSV files (.csv) are allowed'
            }), 400
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    # Extract student information with multiple fallback options (after column standardization)
                    student_roll_number = str(row.get('student_id', row.get('Student ID', row.get('roll_number', row.get('id', ''))))).strip()
                    first_name = str(row.get('first_name', '')).strip()
                    last_name = str(row.get('last_name', '')).strip()
                    
                    # Handle case where name is in a single column (check standardized names first)
                    if not first_name and not last_name:
                        full_name = str(row.get('student_name', row.get('Student Name', row.get('name', '')))).strip()
                        if full_name:
                            name_parts = full_name.split(' ', 1)
                            first_name = name_parts[0]
                            last_name = name_parts[1] if len(name_parts) > 1 else ''
                    
                    # Extract marks with multiple fallback options (check standardized names first)
                    marks_obtained = row.get('score', row.get('Score', row.get('marks', row.get('marks_obtained', 0))))
                    
                    # DEBUG: Log what we extracted
                    logger.info(f"Row {index + 1}: ID='{student_roll_number}', Name='{first_name} {last_name}', Score='{marks_obtained}'")
                    ...
                    if existing_record:
                        ...
                        existing_record.raw_data_from_excel = json.dumps(row.to_dict())
                        records_updated += 1
