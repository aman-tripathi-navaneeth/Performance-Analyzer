...
@analytics_bp.route('/class/performance/<int:year>/<section>', methods=['GET'])
def get_class_performance(year, section):
    """
    Get class performance data for a specific year and section
    GET /api/v1/class/performance/{year}/{section}
    Returns subject-wise class averages and student lists
    """
    try:
        logger.info(f"Fetching class performance for Year {year}, Section {section}")
        
        # Get all subjects for this year and section
        subjects_query = db.session.query(
            Subject.id,
            Subject.subject_name,
            Subject.base_subject_name,
            func.avg(PerformanceRecord.marks_obtained * 100.0 / Assessment.max_marks).label('class_average'),
            func.count(func.distinct(PerformanceRecord.student_id)).label('student_count'),
            func.count(PerformanceRecord.id).label('assessment_count'),
            func.max(PerformanceRecord.marks_obtained * 100.0 / Assessment.max_marks).label('highest_score'),
            func.min(PerformanceRecord.marks_obtained * 100.0 / Assessment.max_marks).label('lowest_score')
        ).join(
            ...
            Subject.id, Subject.subject_name, Subject.base_subject_name
        ).all()
        
        # Process subject performance data
        subjects_performance = []
        for subject in subjects_query:
            class_avg = subject.class_average or 0
            subjects_performance.append({
                'subject_id': subject.id,
                'subject_name': subject.subject_name,
                'base_subject_name': subject.base_subject_name,
                'class_average': round(class_avg, 2),
                'student_count': subject.student_count,
                'assessment_count': subject.assessment_count,
                'highest_score': round(subject.highest_score or 0, 2),
                'lowest_score': round(subject.lowest_score or 0, 2),
                'grade': get_grade_from_percentage(class_avg),
                'performance_level': get_performance_level(class_avg)
            })
        
        # Sort by class average descending
        subjects_performance.sort(key=lambda x: x['class_average'], reverse=True)
        
        # Get overall class statistics
        overall_stats = db.session.query(
            func.avg(PerformanceRecord.marks_obtained * 100.0 / Assessment.max_marks).label('overall_average'),
            func.count(func.distinct(PerformanceRecord.student_id)).label('total_students'),
            func.count(func.distinct(Subject.id)).label('total_subjects'),
        ...
        students_query = db.session.query(
            ...
            Student.first_name,
            Student.last_name,
            func.avg(PerformanceRecord.marks_obtained * 100.0 / Assessment.max_marks).label('student_average'),
            func.count(PerformanceRecord.id).label('assessment_count')
        ).join(
        ...
        for student in students_query:
            student_avg = student.student_average or 0
            students_list.append({
                ...
                'average_percentage': round(student_avg, 2),
                'assessment_count': student.assessment_count,
                'grade': get_grade_from_percentage(student_avg),
                'performance_level': get_performance_level(student_avg)
            })
        
        # Sort students by average descending
        students_list.sort(key=lambda x: x['average_percentage'], reverse=True)
        
        response_data = {
            'year': year,
            'section': section,
            'class_name': f"Year {year} {section}",
            'overall_stats': {
                'overall_average': round(overall_stats.overall_average or 0, 2),
                ...
                'grade': get_grade_from_percentage(overall_stats.overall_average or 0)
            },
            'subjects_performance': subjects_performance,
            'students_list': students_list
        }
        
        logger.info(f"Retrieved class performance: {len(subjects_performance)} subjects, {len(students_list)} students")
        ...
        return jsonify({
            'success': False,
            'error': f'Error fetching class performance: {str(e)}'
        }), 500
