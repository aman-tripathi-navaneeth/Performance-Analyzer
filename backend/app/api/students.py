...
from app import db
from app.models import Student, PerformanceRecord, Assessment, Subject
        ...
        if search_query:
            ...
            query = query.filter(
                or_(
                    ...
                    (Student.first_name + ' ' + Student.last_name).ilike(search_pattern)
                )
            )
...
def get_student_performance(student_id):
    ...
    try:
        ...
        student_performance_data = {
            ...
            'insights': generate_academic_insights(subjects_performance, all_percentages, overall_average)
        }
...
def generate_academic_insights(subjects_performance, all_percentages, overall_average):
    """Generate comprehensive academic insights for a student"""
    if not subjects_performance:
        return {
            'academic_status': 'No performance data available',
            'strengths': [],
            'areas_for_improvement': [],
            'recommendations': ['Upload performance data to get insights'],
            'performance_level': 'Unknown',
            'consistency': 'Unknown'
        }
    
    # Calculate subject averages for analysis
    subject_averages = {
        name: data['statistics']['average_percentage'] 
        for name, data in subjects_performance.items()
    }
    ...
    for subject, avg in subject_averages.items():
        if avg >= 85:
            strengths.append(f"Excellent in {subject} ({avg:.1f}%)")
        elif avg >= 75:
            strengths.append(f"Strong in {subject} ({avg:.1f}%)")
        elif avg < 60:
            weaknesses.append(f"Needs improvement in {subject} ({avg:.1f}%)")
        elif avg < 70:
            weaknesses.append(f"Below average in {subject} ({avg:.1f}%)")
    ...
    if overall_average >= 85:
        ...
        performance_level = "Good Student"
        academic_status = "Solid academic performance with room for growth"
    elif overall_average >= 65:
        performance_level = "Average Student"
        academic_status = "Satisfactory performance, focus needed on weak areas"
    elif overall_average >= 50:
        performance_level = "Below Average Student"
        academic_status = "Requires significant improvement and support"
    else:
        performance_level = "Struggling Student"
        academic_status = "Needs immediate academic intervention and support"
    
    # Analyze consistency
    if all_percentages:
        std_dev = (sum((x - overall_average) ** 2 for x in all_percentages) / len(all_percentages)) ** 0.5
        if std_dev <= 10:
    ...
    if overall_average >= 85:
        ...
        recommendations.extend([
            "Focus on improving weaker subjects to achieve excellence",
            "Maintain strong performance in current strengths",
        ...
        recommendations.extend([
            "Seek immediate academic support and tutoring",
            "Meet with academic counselor to develop improvement plan",
            "Focus on fundamental concepts in all subjects",
            "Consider additional study hours and resources"
        ])
    
    # Add subject-specific recommendations
    if weaknesses:
        worst_subject = min(subject_averages.items(), key=lambda x: x[1])
        recommendations.append(f"Priority focus needed on {worst_subject[0]} (current: {worst_subject[1]:.1f}%)")
    
    # Identify improvement trends
    improving_subjects = [
        name for name, data in subjects_performance.items()
        if data['statistics']['improvement_trend'] == 'improving'
    ]
    
    declining_subjects = [
        name for name, data in subjects_performance.items()
        if data['statistics']['improvement_trend'] == 'declining'
    ]
    
    if improving_subjects:
        recommendations.append(f"Continue positive momentum in: {', '.join(improving_subjects)}")
    ...
    return {
        ...
        'consistency_score': round(100 - min(std_dev * 2, 100), 1) if all_percentages else 0,
        'strengths': strengths[:3],  # Top 3 strengths
        'areas_for_improvement': weaknesses[:3],  # Top 3 areas needing work
        'recommendations': recommendations[:5],  # Top 5 recommendations
        'strongest_subject': max(subject_averages.items(), key=lambda x: x[1])[0] if subject_averages else None,
        'weakest_subject': min(subject_averages.items(), key=lambda x: x[1])[0] if subject_averages else None,
        'improving_subjects': improving_subjects,
        'declining_subjects': declining_subjects,
        'needs_attention': [name for name, avg in subject_averages.items() if avg < 60],
        'academic_summary': f"This student is performing at a {performance_level.lower()} level with an overall average of {overall_average:.1f}%. " +
                          f"{'Strong areas include ' + ', '.join([s.split(' in ')[1].split(' (')[0] for s in strengths[:2]]) + '. ' if strengths else ''}" +
                          f"{'Areas needing attention: ' + ', '.join([w.split(' in ')[1].split(' (')[0] for w in weaknesses[:2]]) + '.' if weaknesses else 'Performance is consistent across all subjects.'}"
    }