...
class Assessment(db.Model):
    ...
    assessment_type = db.Column(db.String(50), nullable=True)  # e.g., "Mid-term", "Final", "Quiz"
    ...
    def __repr__(self):
        return f'<Assessment {self.assessment_name} - {self.subject.subject_name if self.subject else "No Subject"}>'
...
class PerformanceRecord(db.Model):
    ...
    raw_data_from_excel = db.Column(db.Text, nullable=True)  # JSON string instead of JSONB
    ...
    assessment = db.relationship('Assessment', backref='performance_records')
    
    def __repr__(self):
        return f'<PerformanceRecord {self.student.full_name if self.student else "Unknown"} - {self.assessment.assessment_name if self.assessment else "Unknown"}: {self.marks_obtained}>'
    
    def to_dict(self):
        raw_data = None
        if self.raw_data_from_excel:
            try:
                raw_data = json.loads(self.raw_data_from_excel)
            except:
                raw_data = self.raw_data_from_excel
        
        return {
            'id': self.id,
            'marks_obtained': self.marks_obtained,
            'raw_data_from_excel': raw_data,
    ...
    def set_raw_data(self, data):
        """Helper method to set raw data as JSON string"""
        if data:
            self.raw_data_from_excel = json.dumps(data)
    
    def get_raw_data(self):
        """Helper method to get raw data as Python object"""
        if self.raw_data_from_excel:
            try:
                return json.loads(self.raw_data_from_excel)
            except:
                return self.raw_data_from_excel
        return None