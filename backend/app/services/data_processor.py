...
def parse_excel_file(file_path):
    ...
    try:
        ...
        # Load file into DataFrame (Excel or CSV)
        try:
        ...
        try:
            ...
            student_info_cols = ['student_id', 'student_roll_number', 'name', 'first_name', 'last_name', 'student_name', 'email', 'phone', 'remarks']
            numeric_cols = ['score', 'marks', 'marks_obtained', 'percentage']
            
            for col in df.columns:
                # Only convert columns that are likely to be numeric scores
                if any(numeric_keyword in col.lower() for numeric_keyword in ['score', 'mark', 'percentage', 'grade']):
                    try:
                    ...
                    logger.info(f"Keeping column '{col}' as text/object type")
...
def handle_recurring_assessments(subject_name, assessment_name, new_df):
    ...
    import logging
    from app import db
    from app.models import Subject, Assessment, PerformanceRecord, Student
    ...
    try:
        ...
        for record in historical_records:
            ...
            if record.raw_data_from_excel:
                try:
                    raw_data = json.loads(record.raw_data_from_excel)
                    row_data.update(raw_data)
                except (json.JSONDecodeError, TypeError):
                    # Handle case where raw_data_from_excel is not valid JSON
                    pass
